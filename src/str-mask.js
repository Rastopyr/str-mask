...
function prepareOptions(options) {
    ...
    var mask = options.mask,
        ...
        maskChars, maskChar, maskCharFn, isOptional, i;

    //Set maskChars array
    if (typeof mask === 'string') { //use value if mask is not string
        maskChars = [];
...
function setMask(value, maskChars, filler) {
    ...
    var model = '',
        viewValue = '',
        valueChar, maskChar, cursor, isInvalid, isOptionalMask, i, j;
...
mask.curry = function(options) { //Partial application
    options = prepareOptions(options);

    return value => setMask(value, options.m, options.f);
};
